        ...
        public function addComment($parent_id, $article_id, $text) 
        {
            ...
            try 
            {
                ...
                if (($comment_parent->isSaved()) && ($comment_parent->user_id != $this->auth_user->id)) 
                {
                    ...
                    $this->mail->send($user->email, ["user" => $user, "link" => $comment_parent->link], "comment_subscribe");
                }
                return json_encode(["id" => $comment->id, "parent_id" => $comment->parent_id, "user_id" => $this->auth_user->id, "name" => $this->auth_user->name, "avatar" => $this->auth_user->avatar, "text" => $comment->text, "date" => $comment->date]);
            } 
            catch (Exception $e) 
            {
                return false;
